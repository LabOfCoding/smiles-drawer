...
import { Atom, type IBracket } from "./Atom"
import { Edge } from "./Edge"
import { Vector2 } from "./Vector2";
import { Ring } from "./Ring";
import type { ParseTreeNode, AdjacencyMatrix, DistanceMatrix, AdjacencyList, Bridge } from "./types";
...
export class Graph {
   ...
   */
  private _init(node: ParseTreeNode, order: number = 0, parentVertexId: number | null = null, isBranch: boolean = false): void {
    ...
    atom.branchBond = node.branchBond ?? null;
    atom.ringbonds = node.ringbonds;
    atom.bracket = typeof node.atom === 'object' && node.atom.element ? (node.atom as IBracket) : null;
    ...
    if (parentVertexId !== null && parentVertex) {
      ...
      let edge = new Edge(parentVertexId, vertexId, 1);
    ...
    if (atom.bracket && typeof atom.bracket === 'object' && 'chirality' in atom.bracket && atom.bracket.chirality) {
      ...
      stereoHydrogens = atom.bracket.hcount ?? 0;
   ...
   */
  public getEdge(vertexIdA: number, vertexIdB: number): Edge | null {
   ...
   */
  public getEdges(vertexId: number): number[] {
    let edgeIds = Array();
   ...
   */
  public hasEdge(vertexIdA: number, vertexIdB: number): boolean {
   ...
   */
  public getVertexList(): number[] {
    let arr: number[] = Array(this.vertices.length);
   ...
   */
  public getAdjacencyMatrix(): AdjacencyMatrix {
   ...
   */
  public getComponentsAdjacencyMatrix(): AdjacencyMatrix {
    let length = this.vertices.length;
    let adjacencyMatrix = Array(length);
   ...
   */
  public getSubgraphAdjacencyMatrix(vertexIds: number[]): AdjacencyMatrix {
   ...
   */
  public getDistanceMatrix(): DistanceMatrix {
    let length = this.vertices.length;
   ...
   */
  public getSubgraphDistanceMatrix(vertexIds: number[]): DistanceMatrix {
    let length = vertexIds.length;
   ...
   */
  public getSubgraphAdjacencyList(vertexIds: number[]): AdjacencyList {
    let length = vertexIds.length;
    let adjacencyList = Array(length);
   ...
   */
  public getTreeDepth(vertexId: number | null, parentVertexId: number | null): number {
   ...
   */
  public traverseTree(vertexId: number, parentVertexId: number | null, callback: (vertex: Vertex) => void, maxDepth: number = 999999, ignoreFirst: boolean = false, depth: number = 1, visited: Uint8Array | null = null): void {
    if (visited === null) {
   ...
   */
  public kkLayout(vertexIds: number[], center: Vector2, startVertexId: number, ring: Ring, bondLength: number,
    ...
    let energy = function (index: number): [number, number, number] {
    ...
    let highestEnergy = function (): [number, number, number, number] {
      let maxEnergy = 0.0;
      let maxEnergyId = 0;
      let maxDEX = 0.0;
      let maxDEY = 0.0
    ...
    let update = function (index: number, dEX: number, dEY: number): void {
      let dxx = 0.0;
      let dyy = 0.0;
      let dxy = 0.0;
   ...
   */
  private _bridgeDfs(u: number, visited: boolean[], disc: number[], low: number[], parent: (number | null)[], adj: AdjacencyList, outBridges: Bridge[]): void {
    visited[u] = true;
   ...
   */
  public static getConnectedComponents(adjacencyMatrix: AdjacencyMatrix): number[][] {
   ...
   */
  public static getConnectedComponentCount(adjacencyMatrix: AdjacencyMatrix): number {
    let length = adjacencyMatrix.length;
    let visited = new Array(length);
    let count = 0;
   ...
   */
  private static _ccCountDfs(u: number, visited: boolean[], adjacencyMatrix: AdjacencyMatrix): void {
   ...
   */
  private static _ccGetDfs(u: number, visited: boolean[], adjacencyMatrix: AdjacencyMatrix, component: number[]): void {
    for (var v = 0; v < adjacencyMatrix[u].length; v++) {
      let c = adjacencyMatrix[u][v];
