...
export class SvgDrawer {
  private preprocessor: DrawerBase;
  public opts: any;
  private clear: boolean;
  public svgWrapper: SvgWrapper | null;
  ...
  public draw(
    ...
    weightsNormalized: boolean = false
  ): SVGSVGElement {
    if (target === null || target === "svg") {
      target = document.createElementNS("http://www.w3.org/2000/svg", "svg") as SVGSVGElement;
      ...
      target = document.getElementById(target) as SVGSVGElement;
    }
   ...
   * @param {Boolean} infoOnly=false Only output info on the molecule without drawing anything to the canvas.
   */
  drawCanvas(data: any, target: any, themeName: string = "light", infoOnly: boolean = false): void {
    let canvas = null;
  ...
  drawVertices(debug: boolean): void {
    ...
    for (var i = 0; i < graph!.vertices.length; i++) {
      ...
      let dir = vertex.getTextDirection(
        graph!.vertices,
